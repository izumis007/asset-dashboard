...
def upgrade() -> None:
    ...
    op.create_table('assets',
        ...
        sa.Column('asset_class', sa.Enum('CashEq', 'FixedIncome', 'Equity', 'RealAsset', 'Crypto', name='asset_class'), nullable=False),
        sa.Column('asset_type', sa.Enum('Savings', 'MMF', 'Stablecoin', 'GovBond', 'CorpBond', 'BondETF', 'DirectStock', 'EquityETF', 'MutualFund', 'REIT', 'Commodity', 'GoldETF', 'Crypto', name='asset_type'), nullable=True),
        sa.Column('region', sa.Enum('US', 'JP', 'EU', 'DM', 'EM', 'GL', name='region'), nullable=True),
...
def downgrade() -> None:
    op.drop_index('ix_valuation_snapshots_date', table_name='valuation_snapshots')
    ...
    op.drop_index('_symbol_asset_type_uc', table_name='assets')
